...
select c.marca, c.modelo, sum(v.distanciaPercorrida) as quilometragem, e.nome
from carro c, viagem v, empresa e
where c.id = v.idCarro
...
select e.nome, sum(v.distanciaPercorrida) as quilometragem
from carro c, viagem v, empresa e
where c.id = v.idCarro
	and c.idEmpresa = e.id
group by e.nome
...
select e.nome, sum(v.distanciaPercorrida) as somaDistancia, cast(avg(v.distanciaPercorrida) as decimal(10, 2)) as mediaDist
...
select e.nome, max(v.distanciaPercorrida) as quilometragem
from carro c, viagem v, empresa e
where c.id = v.idCarro
	and c.idEmpresa = e.id
group by e.nome
...
select e.nome, count(c.id) as qtdCarros
...
select e.nome, count(v.idCarro) as qtdViagens
from empresa e, viagem v, carro c
where c.id = v.idCarro
	and c.idEmpresa = e.id
group by e.nome
order by (e.nome) asc, (qtdViagens) desc
...
select e.nome, c.marca, c.modelo, v.distanciaPercorrida,
	case when(v.distanciaPercorrida < 1000) then 'R$10,00'
		else 'R$15,00' end as valor
from carro c, viagem v, empresa e
where c.id = v.idCarro
	and c.idEmpresa = e.id
